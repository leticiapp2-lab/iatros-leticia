...
export interface CriteriaGroup {
  ...
  note?: string;
  items: CriteriaItem[];
  /** For display: "Selecione ≥2" etc. */
  minRequired?: number;
  /** If true, all items in the group must be checked (AND logic) */
  allRequired?: boolean;
}
...
// Helper: count checked items in a group
function countInGroup(checked: Set<string>, group: CriteriaGroup): number {
  return group.items.filter((item) => checked.has(item.id)).length;
}

function sumPointsInGroup(checked: Set<string>, group: CriteriaGroup): number {
  return group.items
...
export const interactiveDiseases: InteractiveDisease[] = [
  // ── DEPRESSÃO (DSM-5) ──
  {
    ...
    evaluate: (checked) => {
      const coreCount = countInGroup(checked, interactiveDiseases[0].groups[0]);
      const addCount = countInGroup(checked, interactiveDiseases[0].groups[1]);
  ...
  // ── TAG (DSM-5) ──
  {
    ...
    evaluate: (checked) => {
      const disease = interactiveDiseases[1];
      const reqMet = checked.has("tag-req-1") && checked.has("tag-req-2");
      const sxCount = countInGroup(checked, disease.groups[1]);
  ...
  // ── LES (EULAR/ACR 2019) ──
  {
    ...
    evaluate: (checked) => {
      const disease = interactiveDiseases[2];
      const anaMet = checked.has("les-ana");
      const clinScore = sumPointsInGroup(checked, disease.groups[1]);
      const immScore = sumPointsInGroup(checked, disease.groups[2]);
  ...
  // ── ENXAQUECA SEM AURA (ICHD-3) ──
  {
    ...
    evaluate: (checked) => {
      const disease = interactiveDiseases[3];
      const aMet = checked.has("enx-a1");
      const bMet = checked.has("enx-b1");
      const cCount = countInGroup(checked, disease.groups[2]);
      const dCount = countInGroup(checked, disease.groups[3]);
  ...
  // ── IBS / SII (Roma IV) ──
  {
    ...
    evaluate: (checked) => {
      const disease = interactiveDiseases[4];
      const reqMet = checked.has("ibs-req-1");
      const assocCount = countInGroup(checked, disease.groups[1]);
  ...
  // ── PCOS (Rotterdam) ──
  {
    ...
    evaluate: (checked) => {
      const disease = interactiveDiseases[5];
      const count = countInGroup(checked, disease.groups[0]);
      const met = count >= 2;
  ...
  // ── OSTEOARTRITE JOELHO (ACR) ──
  {
    ...
    evaluate: (checked) => {
      const disease = interactiveDiseases[6];
      const reqMet = checked.has("oa-req-1");
      const critCount = countInGroup(checked, disease.groups[1]);
  ...
  // ── FIBROMIALGIA (AAPT/APS 2019) ──
  {
    ...
    evaluate: (checked) => {
      const disease = interactiveDiseases[7];
      const count = countInGroup(checked, disease.groups[0]);
      const met = count === 3;
  ...
  // ── DIABETES T2 (ADA) ──
  {
    ...
    evaluate: (checked) => {
      const disease = interactiveDiseases[8];
      const sympCount = countInGroup(checked, disease.groups[0]);
      const labCount = countInGroup(checked, disease.groups[1]);
